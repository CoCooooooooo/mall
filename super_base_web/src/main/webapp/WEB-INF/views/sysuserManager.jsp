...
<head>
    ...
    <script>
        ...
        function addSysuser(){
            $.post(
                '${pageContext.request.contextPath}/base/sysuser/add',
                ...
                function(result){
                    if(result.status==1){
                        layer.msg('添加成功',{
                            ...
                            location.href='${pageContext.request.contextPath}/base/sysuser/findAll?pageNum='+${pageInfo.pageNum};
                        })
                    }
        ...
        function showSysuser(id) {
            $.post(
                '${pageContext.request.contextPath}/base/sysuser/findById',
                {'id':id},
                function (result) {
                    if (result.status==1){
        ...
        function modifySys() {
            $.ajax({
                type:'post',
                url:'${pageContext.request.contextPath}/base/sysuser/modify',
                data:{'id': $('#MargerStaffId').val(),
                      'name':$('#MargerStaffname').val(),
                      'loginName':$('#MargerLoginName').val(),
                      'phone':$('#MargerPhone').val(),
                      'email':$('#MargerAdrees').val(),
                      'role':$('#MargerRole').val(),},
                dataType:'json',
        ...
        function modifyStatus(id,btn){
            $.post(
                '${pageContext.request.contextPath}/base/sysuser/modifyStatus',
                {'id':id},
                function(result){
                    if(result.status==1){
...
<div class="panel panel-default" id="adminSet">
    ...
    <div class="panel-body">
        <div class="showmargersearch">
            <form class="form-inline" action="${pageContext.request.contextPath}/base/sysuser/findByParams" method="post" id="frmSearch">
                <%--把页码传到页面上 保持不动显示--%>
...
<div class="modal fade" tabindex="-1" id="myModal-Manger">
    ...
    <div class="modal-dialog">
        ...
        <div class="modal-content">
            ...
            <div class="modal-footer">
                <button class="btn btn-primary doMargerModal" onclick="modifySys()">修改</button>
                <button class="btn btn-primary cancel" data-dismiss="modal">取消</button>
            </div>
        </div>
    </div>
